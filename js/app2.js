...
let originalData = localStorage.getItem('menuProfile') ? JSON.parse(localStorage.getItem('menuProfile')) : []
...
function showInfo(){
    document.querySelectorAll(".menuDetails").forEach(info => info.remove())
    ...
    if(getData.length > 0){
        for(var i=tab_start; i<tab_end; i++){
            ...
            if(staff){
                let createElement = `<tr class = "menuDetails">
                <td>${i+1}</td>
                <td><img src="${staff.picture}" alt="" width="40" height="40"></td>
                <td>${staff.itemNameVal}</td>
                <td>${staff.priceVal}</td>
                <td>${staff.countryVal}</td>
                <td>${staff.categoryVal}</td>
                
                <td>
                    <button onclick="readInfo('${staff.picture}',  '${staff.itemNameVal}', '${staff.priceVal}', '${staff.countryVal}', '${staff.categoryVal}')"><i class="fa-regular fa-eye"></i></button>

                    <button onclick="editInfo('${i}', ${staff.picture}',  '${staff.itemNameVal}', '${staff.priceVal}', '${staff.countryVal}', '${staff.categoryVal}')"><i class="fa-regular fa-pen-to-square"></i></button>
                ...
                menuInfo.innerHTML += createElement
...
function readInfo(pic, itemName, price, country, category){
    imgInput.src = pic
...
function editInfo(id, pic, itemName, price, country, category){
    isEdit = true
    editId = id
...
function deleteInfo(index){
    if(confirm("Aer you sure want to delete?")){
        ...
        localStorage.setItem("menuProfile", JSON.stringify(originalData));
...
filterData.addEventListener("input", ()=> {
    ...
    if(searchTerm !== ""){
        ...
        getData = JSON.parse(localStorage.getItem('menuProfile')) || []
    }
